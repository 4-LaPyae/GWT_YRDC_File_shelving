<?php
	$movepath = '';
	$pgTitle = 'စာဖိုင်တွဲ အ၀င်အထွက် ပြင်ဆင်ခြင်း';
	$currentPg = 'Folder Transaction Edit';
	...
	{
		$transaction_id = clean($_GET['transaction_id']);
		if($transaction_id == 0)
			$urlpath_flag = false;
		else
		{
			$urlpath_flag = true;
			$row = $transaction_bol->select_transaction_byid($transaction_id);
			$taken_date1 = ($row['taken_date']!="0000-00-00")?date('d-m-Y H:i:s', strtotime(trim($row['taken_date']))):'';
			$folder_id = clean_jscode($row['folder_id']);
	...
	{
		$transactionid = clean($_POST['hidtransactionid']);
		...
		if( $transaction_bol->check_duplicate_transaction_name($taken_employeeid, $taken_employee_name, $taken_designation, $taken_department, $transaction_id) )
			$errors[] = 'ဤစာဖိုင်တွဲ အ၀င်အထွက်စာရင်းရှိနှင့်ပြီးဖြစ်သည်';
		
		if( count($errors) ==  0)
		{
			$success = FALSE;
			$transaction_info->set_transaction_id($transaction_id);
			...
			$transaction_info->set_modified_by($userid);
			$success = $transaction_bol->save_transaction($transaction_info);
			if( $success == TRUE )
			{
				if($folder_bol->update_folder_status($folder_id, '2'))
				{
					$_SESSION['transaction_msg'] = "စာဖိုင်တွဲ အ၀င်အထွက် ပြင်ဆင်ခြင်းအောင်မြင်သည်";
...
<script language="javascript">
	...
	function update_transaction_form()
	{
		// Add_Validation();
		if($('#frm_transaction_setup').valid())
		{
...
<form name="frm_transaction_setup" id="frm_transaction_setup" method="POST" class="form-material form-horizontal" enctype="multipart/form-data">
	<input type="hidden" name="hidtransactionid" id="hidtransactionid" value="<?php echo $transaction_id?>">
		...
		if( isset($pageenablearr["Edit"]) || $usertypeid==0 )
		{
			echo '<div class="form-group mt-4 row" id="divbuttons">
				<div class="offset-md-4 col">
					<input type="submit" class="btn btn-success" id="btnupdate" name="btnupdate" value="ပြင်ဆင်မည်" onclick="update_transaction_form();" />
					<input type="button" class="btn btn-outline-secondary" value="မပြင်ဆင်ပါ" onclick="window.location=\'transaction_list.php?folder_id='.$folder_id.'\'" />
				</div>
			</div>';
		}
	?>
