<?php
	...
	{
		if( isset($_POST['delete_file_transaction_id']) )
		{  
			$json_return_arr['sessionexpire'] = 0;
			$file_transaction_id = (int)($_POST['delete_file_transaction_id']);
			if( $file_transaction_bol->delete_file_transaction($file_transaction_id) )
			{
				$json_return_arr['success'] = 1;
				$json_return_arr['alertclass'] = 'alert alert-success';
				$json_return_arr['message'] = 'စာများ အ၀င်အထွက်စာရင်း ပယ်ဖျက်ခြင်း အောင်မြင်သည်';
			}
			else
			{
				$json_return_arr['success'] = 0;
				$json_return_arr['alertclass'] = 'alert alert-danger';
				$json_return_arr['message'] = 'စာများ အ၀င်အထွက်စာရင်း ပယ်ဖျက်ခြင်း မအောင်မြင်ပါ';
			}
		}
	}
	...
	{  
		if( isset($_POST['given_file_transaction_popup']))
		{
			$json_return_arr['sessionexpire'] = 0;
			
			$file_transaction_id = clean($_POST['given_file_transaction_popup']);
			$file_id = clean($_POST['file_id']);
			
			$return_str = '<div class="modal-body">		
									<form id="frm_file_transaction_setup" name="frm_file_transaction_setup" class="form-material form-horizontal" role="form" method="post" enctype="multipart/form-data" action="">
										<div id="alert_msg" class="error"></div>
										<div class="form-group row">
											<input type="hidden" id="hidfile_id" name="hidfile_id" value="' .$file_id . '" >
											<input type="hidden" id="hidfiletransactionid" name="hidfiletransactionid" value="' .$file_transaction_id . '" >
								...
								<div class="modal-footer" id="divbuttons">
									<button type="button" class="btn btn-success" id="btnnew" name="btnnew" onclick="save_given_file_transaction()">သိမ်းမည် </button>
	...
	if((isset($_GET['authaction']) && $_GET['authaction']=='backup'))
	{
		if( isset($_POST['file_transaction_id']) && isset($_POST['file_id']) && isset($_POST['given_date']))
		{
			$file_id = clean_jscode($_POST['file_id']);
			$file_transaction_id = clean_jscode($_POST['file_transaction_id']);
			...
			{
				$json_return_arr['sessionexpire'] = 0;			
				if ( $file_transaction_bol->save_given_date($given_date, $file_transaction_id) )
				{
					if($file_bol->update_file_status($file_id, '1'))
					{
						$json_return_arr['success'] = 1;
						$json_return_arr['alertclass'] = 'alert alert-success';
						$json_return_arr['message'] = 'စာများအ၀င်အထွက်ပြန်သွင်းခြင်း အောင်မြင်သည်';
					}
				}
				else
				{
					$json_return_arr['success'] = 0;
					$json_return_arr['alertclass'] = 'alert alert-danger';
					$json_return_arr['message'] = 'စာများအ၀င်အထွက်ပြန်သွင်းခြင်း မအောင်မြင်ပါ';
				}
			}
			else
			{
