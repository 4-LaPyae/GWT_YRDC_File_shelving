<?php
	$movepath = '';	
	$pgTitle = 'စာဖိုင် အနှစ်ချုပ်စာရင်း';
	$currentPg = 'File Summary Report';
...
<script language="javascript">
	...
	var cookie_name = 'summary_file_report_list';
	
	function downloadfile()
	{
		postRedirectURL("file_summary_report_export.php", {"filter":getFilter(), "cri_text":cri_text});
	}
	...
	jQuery(document).ready(function()
	{
		...
		jQuery("#frm_file_summary_report_list").submit(function(e)
		{
			if(jQuery('#frm_file_summary_report_list').valid())
			{
				getloading();
				jQuery("#frm_file_summary_report_list").unbind('submit').submit();
				return true;
			}
		...
		oTable = jQuery('#dtList').dataTable(
		{
			...
			"sAjaxSource": "file_summary_report_getlist.php",
			columns: [
...
<form id="frm_file_summary_report_list" name="frm_file_summary_report_list" method="POST">
