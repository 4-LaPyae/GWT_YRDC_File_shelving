<?php
	...
	$download_reportname = "folder_summary_report_export";
	$reporttitle = 'Folder Summary Report/စာဖိုင်တွဲ အနှစ်ချုပ်စာရင်း';
	...
	$SortingCols = '';
	if( isset($_SESSION['SESS_SUMMARY_FOLDER_SORTINGCOLS_MLR']) )
		$SortingCols = $_SESSION['SESS_SUMMARY_FOLDER_SORTINGCOLS_MLR'];
	...
	$header_colunm = array();
	$type = "list";
	$datasource = $report_bol->select_folder_summary_report_list(0, 0, $SortingCols, $cri_arr);
	...
	if( $datasource->rowCount() > 0 )
	{
		...
		// စုစုပေါင်း
		$line++;
		$response[] = total_folder_summary_report();
		$sheet->getStyle("B$line")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		$sheet->getStyle("C$line")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		$sheet->getStyle("D$line")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		$sheet->getStyle("E$line")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
