<?php
	$movepath = '';
	$pgTitle = 'စာဖိုင်ဖျက်သိမ်းမှု့ အသစ်ထည့်ခြင်း';
	$currentPg = 'File Delete Add New';
	...
	if(isset($_POST['btnsave']))
	{
		...
		if( count($errors) ==  0)
		{
			...
			$file_info->set_destroy_order_employeeid($destroy_order_employeeid);
			$file_info->set_destroy_order_employee_name($destroy_order_employee_name);
			$file_info->set_destroy_order_designation($destroy_order_designation);
			$file_info->set_destroy_order_department($destroy_order_department);
			$file_info->set_destroy_duty_employeeid($destroy_duty_employeeid);
			$file_info->set_destroy_duty_employee_name($destroy_duty_employee_name);
			$file_info->set_destroy_duty_designation($destroy_duty_designation);
			$file_info->set_destroy_duty_department($destroy_duty_department);
			$file_info->set_destroy_date($destroy_date);
			$file_info->set_destroy_order_no($destroy_order_no);
			$file_info->set_destroy_remark($destroy_remark);
			$file_info->set_modified_by($userid);
			$success = $file_bol->update_file_destroy($file_info);
			if( $success == TRUE )
			{
				$_SESSION['file_msg'] = "ဖိုင်ဖျက်သိမ်းမှု့ အသစ်ထည့်ခြင်းအောင်မြင်သည်";
...
<script language="javascript">
	...
	$(document).ready(function()
	{
		...
		$("#frm_file_delete_setup").submit(function(e)
		{
			if($('#frm_file_delete_setup').valid())
			{
				getloading();
				$("#frm_file_delete_setup").unbind('submit').submit();				
				return true;
			}
		});
	});
	...
	function go_to_order_form()
	{
		...
		$.post('file_exec.php', {'add_order_popup':"add_order_popup"}, function(result)
		{
	...
	function go_to_duty_form()
	{
		...
		$.post('file_exec.php', {'add_duty_popup':"add_duty_popup"}, function(result)
		{
			select_data_exec_call_back(result);
			$('.scrollable').height($(window).height() - 220).css('margin-right', -15);
		}, 'json');
	...
	{
		$("#frm_file_delete_setup").validate(
		{
			'rules':{
	...
	function save_file_delete_form()
	{
		Add_Validation();
		if($('#frm_file_delete_setup').valid())
		{
...
<form name="frm_file_delete_setup" id="frm_file_delete_setup" method="POST" class="form-material form-horizontal" enctype="multipart/form-data">
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">ဖျက်သိမ်းရက်စွဲ</label>
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">အကြာင်းအရာ</label>
		<div class="col-lg-6 col-md-7">			
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3">အမိန့်အမှတ်</label>
		<div class="col-lg-6 col-md-7">
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">ခွင့်ပြုသူ</label>
		<div class="col-lg-6 col-md-7">
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">ကိုယ်ပိုင်အမှတ်</label>
		<div class="col-lg-6 col-md-7">
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">အမည်</label>
		<div class="col-lg-6 col-md-7">
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">ရာထူး</label>
		<div class="col-lg-6 col-md-7">
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">ဌာန</label>
		<div class="col-lg-6 col-md-7">
	...
	<div class="form-group row">
		<label class="col-form-label col-md-3 required">တာ၀န်ခံရသူ</label>
		<div class="col-lg-6 col-md-7">
		...
		if( isset($pageenablearr["Add"]) || $usertypeid==0 )
		{
			echo '<div class="form-group mt-4 row" id="divbuttons">
				<div class="col-12 col-lg-10 text-right">
					<input type="submit" class="btn btn-success" id="btnsave" name="btnsave" value="သိမ်းမည်" onclick="save_file_delete_form();" />
