<?php
	...
	$download_reportname = "burn_file_report_export";
	$reporttitle = 'File Burn Report/စာဖိုင် ဖျက်သိမ်းမှုစာရင်း';
	...
	$SortingCols = '';
	if( isset($_SESSION['SESS_BURN_FILE_SORTINGCOLS_MLR']) )
		$SortingCols = $_SESSION['SESS_BURN_FILE_SORTINGCOLS_MLR'];
	...
	$header_colunm = array();
	$type = "list";
	$datasource = $report_bol->select_file_burn_report_list(0, 0, $SortingCols, $cri_arr);
	...
	PHPExcelMergeCenter($sheet,"$curcol$line:$curcol".($line));
	$sheet->setCellValue($curcol.$line, 'စာအမှတ်');
	$curcol = Next_Excel_Col($curcol);
	
	$sheet->getStyle($curcol.$line)->getFont()->setBold(true);	
	$sheet->mergeCells("K2:K3");
	PHPExcelMergeCenter($sheet,"$curcol$line:$curcol".($line));
	$sheet->setCellValue($curcol.$line, 'ရက်စွဲ');
	$curcol = Next_Excel_Col($curcol);
	...
	PHPExcelMergeCenter($sheet,"$curcol$line:$curcol".($line));
	$sheet->setCellValue($curcol.$line, 'ဖိုင်လုံခြုံမှု့အဆင့်အတန်း');
	$curcol = Next_Excel_Col($curcol);
	
	$sheet->getStyle($curcol.$line)->getFont()->setBold(true);	
	$sheet->mergeCells("N2:N3");
	PHPExcelMergeCenter($sheet,"$curcol$line:$curcol".($line));
	$sheet->setCellValue($curcol.$line, 'လုပ်ငန်းအမျိုးအစား');
	...
	while($aRow = $datasource->getNext())
	{
		...
		$sheet->getStyle("N$line")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
	}
