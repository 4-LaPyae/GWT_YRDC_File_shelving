<?php
	$movepath = '';	
	$pgTitle = 'စာဖိုင် ဖျက်သိမ်းမှုစာရင်း';
	$currentPg = 'File Burn Report';
...
<script language="javascript">
	...
	var cookie_name = 'burn_file_report_list';
	
	function downloadfile()
	{
		postRedirectURL("burn_file_report_export.php", {"filter":getFilter(), "cri_text":cri_text});
	}
	...
	jQuery(document).ready(function()
	{
		...
		jQuery("#frm_file_burn_report_list").submit(function(e)
		{
			if(jQuery('#frm_file_burn_report_list').valid())
			{
				getloading();
				jQuery("#frm_file_burn_report_list").unbind('submit').submit();
				return true;
			}
		...
		oTable = jQuery('#dtList').dataTable(
		{
			...
			"sAjaxSource": "burn_file_report_getlist.php",
			columns: [
...
<form id="frm_file_burn_report_list" name="frm_file_burn_report_list" method="POST">
	...
	<table id="dtList" name="dtList" class="table dataTable table-striped table-hover nowrap" cellspacing="0" width="100%">
		<thead>
			<tr>
				...
				<th rowspan='2'>စာအမှတ်</th>
				<th rowspan='2'>ရက်စွဲ</th>
				<th rowspan='2'>အကြောင်းအရာ</th>
				<th rowspan='2'>ဖိုင်လုံခြုံမှုအဆင့်အတန်း </th>
				<th rowspan='2'>လုပ်ငန်းအမျိုးအစား </th>
			</tr>
			<tr>
		...
		<tbody>
			<tr>
				<td colspan="14" align="center">Loading data from server</td>
			</tr>
		</tbody>
	</table>
	<?php
