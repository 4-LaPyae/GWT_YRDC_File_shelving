...
//require_once($movepath ."adminauth.php");
...
function SendSaveAsFileHeaderIfNeeded() {
	...
	$downloadfilename = phpthumb_functions::SanitizeFilename(@$_GET['sia'] ? $_GET['sia'] : (@$_GET['down'] ? $_GET['down'] : 'gwt_image'));//ktw edit
	if (@$downloadfilename) {
...
if (!empty($PHPTHUMB_CONFIG['high_security_enabled'])) {
	...
	} elseif ($_GET['hash'] != md5(str_replace('&hash='.$_GET['hash'], '', rawurldecode($_SERVER['QUERY_STRING'])).$PHPTHUMB_CONFIG['high_security_password'])) {
...
if(isset($_GET['uploadtype'])){
	...
	if (array_key_exists($uploadtype, $g_upload_path_type)) {
		// file is okay continue
		$base_g_upload_path = $g_upload_path_type[$uploadtype];
	} else {
		 die("Invalid type $uploadtype , error code 0");
	} 
	
	$tmpfoldername = ""; 
	if(isset($_GET['tmpfoldername'])){
		$tmpfoldername = $_GET['tmpfoldername']; 
	}

	if($tmpfoldername!=="")
		$base_g_upload_path .= "".$tmpfoldername."/"; 

	$_GET['src'] = "/".$base_g_upload_path.$_GET['src']; //echo $_GET['src']; exit();
}
////////////////////////////////////////////////////////////////
// Debug output, to try and help me diagnose problems
...
// deprecated: 'err', 'file', 'goto',
$allowedGETparameters = array('src', 'new', 'w', 'h', 'wp', 'hp', 'wl', 'hl', 'ws', 'hs', 'f', 'q', 'sx', 'sy', 'sw', 'sh', 'zc', 'bc', 'bg', 'bgt', 'fltr', 'xto', 'ra', 'ar', 'aoe', 'far', 'iar', 'maxb', 'down', 'phpThumbDebug', 'hash', 'md5s', 'sfn', 'dpi', 'sia', 'nocache', 'filetime' , 'uploadtype' , 'tmpfoldername');
