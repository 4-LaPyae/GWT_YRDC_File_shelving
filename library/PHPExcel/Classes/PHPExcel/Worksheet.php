...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    */
	...
	public function fromArray_ValueExplicit($source = null, $nullValue = null, $startCell = 'A1', $strictNullComparison = false) {
		if (is_array($source)) {
			// start coordinate
			list ($startColumn, $startRow) = PHPExcel_Cell::coordinateFromString($startCell);
			...
			foreach ($source as $rowData) {
				...
				foreach($rowData as $cellValue) {
					if ($strictNullComparison) {
						if ($cellValue !== $nullValue) {
							// Set cell value
							$this->getCell($currentColumn . $startRow)->setValueExplicit($cellValue, PHPExcel_Cell_DataType::TYPE_NULL);
						}
					} else {
						if ($cellValue != $nullValue) {
							// Set cell value
