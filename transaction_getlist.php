<?php
	...
	if(isset($_GET['folder_id']))
	{
		...
		$cri_str = "WHERE 1=1 AND folder_id = :folder_id ";
		$param = array(":folder_id"=>$folder_id);
	}
	...
	$rResult = $transaction_bol->select_folder_transaction_list($DisplayStart, $DisplayLength, $SortingCols, $cri_arr);	
	...
	{
		$transaction_id = htmlspecialchars($aRow['transaction_id']);
		...
		if ( isset($pageenablearr["Edit"]) || $usertypeid == 0 )
			$action .= "<a href='transaction_update.php?transaction_id=$transaction_id' title='Edit' class='d-inline-flex'><svg class='icon i-xs p-1 text-black'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='js/symbol-defs.svg#icon-edit' /></svg></a>";
		
		if ( isset($pageenablearr["Delete"]) || $usertypeid == 0 )
			$action .= "<a href id='icodelete$transaction_id' title='Delete' data-toggle='modal' data-target='#modal-delete' onclick='delete_transaction($transaction_id, \"". rawurlencode($taken_employee_name) ."\")' class='d-inline-flex'><svg class='icon i-xs p-1 text-black'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='js/symbol-defs.svg#icon-trash' /></svg></a>";
		
		if($given_date == '')
		{
			if ( isset($pageenablearr["Backup"]) || $usertypeid == 0 )
				$action .= "<a href title='Backup' data-toggle='modal' data-target='#modal-backup' onclick='create_add_given_date_popup($transaction_id, $folder_id)' class='d-inline-flex'><svg class='icon i-xs p-1 text-black'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='js/symbol-defs.svg#icon-back-file' /></svg></a>";
		}
