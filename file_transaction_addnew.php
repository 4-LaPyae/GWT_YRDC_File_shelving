<?php
	$movepath = '';
	$pgTitle = 'စာဖိုင် အ၀င်အထွက် အသစ်ထည့်ခြင်း';
	$currentPg = 'File Transaction Add New';
	...
	if(isset($_POST['btnsave']))
	{
		...
		if( $file_transaction_bol->check_duplicate_file_transaction_name($taken_employeeid, $taken_employee_name, $taken_designation, $taken_department) )
		...
		if( count($errors) ==  0)
		{
			...
			$file_transaction_info->set_created_by($userid);
			if ( $file_transaction_bol->save_file_transaction($file_transaction_info) )
			{
...
<script language="javascript">
	...
	function save_file_transaction_form()
	{
		// Add_Validation();
		if($('#frm_file_transaction_setup').valid())
		{
		...
		if( isset($pageenablearr["Add"]) || $usertypeid==0 )
		{
			echo '<div class="form-group mt-4 row" id="divbuttons">
				<div class="offset-md-4 col">
					<input type="submit" class="btn btn-success" id="btnsave" name="btnsave" value="သိမ်းမည်" onclick="save_file_transaction_form();" />
					<input type="button" class="btn btn-outline-secondary" value="မသိမ်းပါ" onclick="window.location=\'file_transaction_list.php?file_id='.$file_id.'\'" />
				</div>
			</div>';
		}
	?>
